...
Adapted for compact JSON format (flat matches + simple standings)
"""
...
# LOAD COMPACT JSON
# =========================================================================
print("Loading C++ mid-tournament hybrid simulation matrices (compact JSON)...")
try:
...
# Reorganize matches into groups
matches_by_group = {}
for match in data["matches"]:
    grp = match["group"]
    matches_by_group.setdefault(grp, []).append(match)

standings = data["standings"]   # dict of group -> list of {team, pts, pos: [1st,2nd,3rd,4th]}
...
for idx, g_name in enumerate(groups):
    ...
    ax = axes1[idx]
    group_standings = standings[g_name]
    
    # Sort by expected points descending
    teams_data = sorted(group_standings, key=lambda x: x["pts"], reverse=True)
    team_names_raw = [t["team"] for t in teams_data]
    
    p1 = [t["pos"][0] * 100 for t in teams_data]
    p2 = [t["pos"][1] * 100 for t in teams_data]
    p3 = [t["pos"][2] * 100 for t in teams_data]
    p4 = [t["pos"][3] * 100 for t in teams_data]
    ...
    for i, t in enumerate(teams_data):
        ax.text(103, i, f"{t['pts']:.2f} XP", va='center', ha='left', fontsize=10, color='#4a4e69', fontweight='bold', zorder=3)
...
for idx, g_name in enumerate(groups):
    ...
    matches = matches_by_group[g_name]
    ...
    for m_idx, m in enumerate(matches):
        ...
        is_played = m.get("played", False)
        # Extract probabilities (new field names)
        home_prob = m.get("p_home", 0.333) * 100
        draw_prob = m.get("p_draw", 0.333) * 100
        away_prob = m.get("p_away", 0.333) * 100
        
        if is_played:
            # Use the actual score from the JSON (should be present for played matches)
            score = m.get("score", f"{int(m['home_xg'])}-{int(m['away_xg'])}")
            if not score.endswith("(FT)"):
                score += " (FT)"
            # For played matches, probabilities are 0 or 1 based on result, but we'll just show 100% on winning side
            # Keep probabilities as they are? The JSON for played matches might not have p_home etc. So we compute them.
            # However, we'll stick to the provided probabilities (if any). But if missing, compute from home_xg > away_xg.
            if "p_home" not in m or m["p_home"] is None:
                home_prob = 100.0 if m['home_xg'] > m['away_xg'] else 0.0
                draw_prob = 100.0 if m['home_xg'] == m['away_xg'] else 0.0
                away_prob = 100.0 if m['home_xg'] < m['away_xg'] else 0.0
        else:
            score = m.get("score", "--")
